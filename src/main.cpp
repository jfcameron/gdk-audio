...
// learning how to decode ogg vorbis and provide to openal context
...
{
    sound_imp(const std::vector<const unsigned char> aOggVorbisData);
    sound_imp(const std::string &aOggVorbisFileName);
};

sound_imp::sound_imp(const std::string &aOggVorbisFileName) : sound_imp([]()
{
    return std::vector<const unsigned char>();
}())
{}

sound_imp::sound_imp(const std::vector<const unsigned char> aOggVorbisData)
{
    int error = 0;
    stb_vorbis_alloc *alloc;
    stb_vorbis *vorbis = stb_vorbis_open_memory(&aOggVorbisData[0], aOggVorbisData.size(), &error, alloc);
...
int main(int argc, char **argv)
{
    ...
    ALCdevice *const device = []()
    {
        ...
        //else
        {
            //TODO offer a choice of devices? -> once better idea of api, build RAII abstraction, ctor option will be device preference
        }
    }();
    ...
    //ifstream in2(arg, ios_base::in | ios_base::binary );
    //sound_imp();
